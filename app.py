...
if not st.session_state.authenticated:
    ...
    with col_center:
        ...
        st.markdown("<h2 style='text-align: center; margin-bottom: 25px;'>Utility Infrastructure Log In</h2>", unsafe_allowed_html=True)
        ...
        with st.form("login_form"):
            ...
            if submit:
                # Basic authentication verification logic - Change as needed
...
if st.session_state.selected_plant is None:
    ...
    with grid_stp:
        st.markdown("<div style='background:#1e293b; padding:25px; border-radius:12px; border:1px solid #334155; text-align:center;'><h3>⚙️ STP</h3><p style='color:#64748b;'>Sewage Treatment Network</p></div>", unsafe_allowed_html=True)
    ...
    with grid_irr:
        st.markdown("<div style='background:#1e293b; padding:25px; border-radius:12px; border:1px solid #334155; text-align:center;'><h3>🌱 Irrigation</h3><p style='color:#64748b;'>Water Distribution Grid</p></div>", unsafe_allowed_html=True)
    ...
    with grid_dcp:
        st.markdown("<div style='background: linear-gradient(135deg, #0369a1 0%, #0f172a 100%); padding:25px; border-radius:12px; border:1px solid #0284c7; text-align:center;'><h3>❄️ DCP</h3><p style='color:#e2e8f0;'>District Cooling Plant</p></div>", unsafe_allowed_html=True)
        if st.button("Open DCP Network", key="btn_dcp", use_container_width=True):
            st.session_state.selected_plant = "DCP"
            st.rerun()
            
    with grid_pot:
        st.markdown("<div style='background:#1e293b; padding:25px; border-radius:12px; border:1px solid #334155; text-align:center;'><h3>💧 Potable</h3><p style='color:#64748b;'>Drinking Water Infrastructure</p></div>", unsafe_allowed_html=True)
...
if st.session_state.selected_plant == "DCP":
    ...
    with st.sidebar:
        ...
        st.markdown("<h4 style='color: #38bdf8; margin-bottom: 20px;'>DCP System Active</h4>", unsafe_allowed_html=True)
    ...
    if menu == "📊 Performance Dashboard":
        ...
        st.markdown("<p style='color:#94a3b8;'>Real-time operational summaries for District Cooling Plant Network</p>", unsafe_allowed_html=True)
    ...
    # SECTION B: DATA INPUT FORM VIEW
        ...
        st.markdown("<p style='color:#94a3b8;'>Input precise field metrics directly into the shared cloud tables</p>", unsafe_allowed_html=True)
